...
export function ErrorState({title = "Oops, no se pudieron cargar los datos", subtitle = "Por favor, inténtalo de nuevo más tarde.", message, onRetry, variant = "card", className = "",}: ErrorStateProps) {
    const handleAction = onRetry || (() => window.location.reload());

    if (variant === "compact") {
        return (
            <div className={`group relative flex items-center justify-between p-4 rounded-xl bg-white border border-border-divider shadow-sm overflow-hidden transition-all hover:border-red-200 ${className}`}>
            ...
            <span className="text-xs font-semibold text-text-title">
              {title !== "No se pudieron cargar los datos" ? title : "Algo no salió como esperábamos"}
                ...
                >
                    <RefreshCw size={13} className="text-text-subtle group-hover:rotate-180 transition-transform duration-500" />
    ...
    return (
        <Card className={`w-full w-full ${className}`}>
            <Card.Content className="flex flex-col items-center justify-center py-10 px-4 text-center gap-3">
                ...
                <div className="mt-2 group">
                    <Button
                        ...
                        variant="primary"
                        icon={<RefreshCw size={15} className="group-hover:rotate-180 transition-transform duration-500" />}
                        onClick={handleAction}
                    />
                </div>
            </Card.Content>
